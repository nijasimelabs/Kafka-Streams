...
object TrafficStreamProcessing {
  ...
  val aggregate_values:ObjectNode = JsonNodeFactory.instance.objectNode();
   ...
   //define stream here 
   val trafficStream: KTable[String, JsonNode] = builder.table(trafficTopic, Consumed.`with`(stringSerde, jsonSerde));
   ...
   trafficStream.foreach(new ForeachAction[String, JsonNode]() {
